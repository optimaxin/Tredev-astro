...
export function Categories() {
  return (
    <SectionContainer>
      ...
      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3 lg:auto-rows-[13rem]">
        {categories.map((cat, i) => {
          ...
          return (
            <Link
              key={cat.title}
              href={cat.href}
              className={`group relative overflow-hidden rounded-xl bg-gradient-to-br ${cat.gradient} p-6 text-white transition-transform duration-200 hover:-translate-y-1 hover:scale-[1.02] ${isFeatured ? "sm:col-span-2 lg:col-span-2 lg:row-span-2" : ""}`}
            >
